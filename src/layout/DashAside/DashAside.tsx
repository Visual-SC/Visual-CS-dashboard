      ...
      <ul className="flex flex-col items-center gap-6" ref={container}>
        ...
        {navItems.map((item, index) => (
          <li key={item.to}>
            <Link
              to={item.to}
              className="item relative inline-flex items-center gap-2 justify-between w-65"
              onClick={() => handleClickGSAP(index)}
            >
              <div className="relative">
                <span className="fontA text-dark-green text-p-16 font-semibold">{item.label}</span>
                <span className="fontB absolute -top-10  left-0 opacity-0 text-dark-green text-p-16 font-semibold">{item.label}</span>
              </div>
